package com.sundy.sms.service.spy;
...
public class SqlCostInterceptor implements Interceptor {
     ...
     * 处理参数为List的场景
     */
    private String handleListParameter(String sql, Collection<?> col) throws IllegalAccessException {
            ...
            for (Object obj : col) {
                ...
                if (isPrimitiveOrPrimitiveWrapper(objClass)) {
                    ...
                    for (Field field : fields) {
                        ...
                        String property = CaseFormat.LOWER_CAMEL.to(CaseFormat.LOWER_UNDERSCORE, propertyName);
                        sqls[i] = sqls[i].replace("," + property + "=?", "," + property + "=" + value);
                        sqls[i] = sqls[i].replace(" " + property + "=?", " " + property + "=" + value);
                    }
    ...
    private boolean isStrictMap(Class<?> parameterObjectClass) {
        return "org.apache.ibatis.session.defaults.DefaultSqlSession$StrictMap".equalsIgnoreCase(parameterObjectClass.getName());
    }
